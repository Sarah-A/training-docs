...
namespace Exercises
{
    public static class Select
    {
        ...
        public static IEnumerable<int> GetNumbers(IEnumerable<object> objects)
        {
            ...
            var intsFromStrings = objects.OfType<string>()
                .Where(obj => int.TryParse(obj, out parsedInt))
                .Select(obj => {
                    Console.WriteLine($"select - {selectIndex++}");
                    int parsedInt;
                    int.TryParse(obj, out parsedInt);
                    return parsedInt;
            });

            Console.WriteLine("before uniting the lists");

            return ints
                .Concat(intsFromStrings)
                .OrderBy(value =>
                {
                    Console.WriteLine($"OrderBy - {orderIndex++}");
                    return value;
                });
        }
