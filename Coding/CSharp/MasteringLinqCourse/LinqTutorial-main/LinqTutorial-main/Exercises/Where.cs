...
namespace Exercises
{
    public static class Where
    {
        ...
        {
            return students.Where( student =>  (student.Marks.Any() ? student.Marks.Average() : 0) > 4.6);
        }
        ...
        {
            return words.Where((word, index) => word.StartsWith((index+1).ToString()));
        }
            ...
            IEnumerable<Person> people)
        {
            return people.Where(person => 
                person.Pets.Count(pet => pet.PetType == PetType.Fish) > 1);
        }
